...
export function seriesToGraphable(series, globalFirstDate) {
  ...
  const totalDays = (Number(new Date(series[series.length - 1].dateStart)) - Number(globalFirstDate)) / msPerDay
  const daysWithoutData = Math.max(
    (Number(localFirstDate) - Number(globalFirstDate)) / msPerDay,
    totalDays - series.length
  )
  const res = Array(daysWithoutData).fill(0, 0, daysWithoutData)
  const unknownDays = []
