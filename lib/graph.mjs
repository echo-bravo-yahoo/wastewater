...
export function outputArraysFromSeries(series) {
  ...
  for (let i = 0; i < Object.keys(series).length; i++) {
    ...
    console.log(JSON.stringify(seriesToGraphable(series[Object.keys(series)[i]], firstDate), null, 2) + '\n')
  }
}
...
export function graphAllSeries(series) {
  ...
  for (let i = 0; i < Object.keys(series).length; i++) {
    console.log(headerFromSeries(series[Object.keys(series)[i]]))
    console.log(sparkly(seriesToGraphable(series[Object.keys(series)[i]], firstDate), { maximum: 100 }) + '\n')
  }
}
