...
export function outputArraysFromSeries(series) {
  ...
  for (let i = 0; i < Object.keys(series).length; i++) {
    ...
    console.log(JSON.stringify(seriesToGraphable(series[Object.keys(series)[i]], firstDate).graphable, null, 2) + lineEnd)
  }
}
...
export function graphAllSeries(series) {
  ...
  for (let i = 0; i < Object.keys(series).length; i++) {
    ...
    const { graphable, unknown } = seriesToGraphable(series[Object.keys(series)[i]], firstDate)
